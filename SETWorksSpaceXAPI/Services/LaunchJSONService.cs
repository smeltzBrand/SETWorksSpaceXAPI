...
namespace SETWorksSpaceXAPI.Services
{
    public class LaunchJSONService : ILaunchJSONService
    {
        public async Task<List<Launch>> GetLaunchDataAsync()
        {
            ...
            //A temporary solution to a SpaceX API bug, see #2 below 
            for (int i = 0; i < 3; i++)
            {
                if (payloads.Count() == 0)
                {
            ...
            try
            {
                ...
                //#2 There is currently a bug with the SpaceX website that returns a "too many requests" error 50% of the time
                // So, I had to use this for loop for the time being.
                for (int i = 0; i < 3; i++)
                {

                    if (rootLaunches.Count() == 0)
                    {
